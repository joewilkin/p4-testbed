...
const bit<32> UPPERLIMIT = 0xD05 ; //0x411B = 16667 microseconds or 60 pps
const bit<32> LOWERLIMIT = 0x1388 ; //0x61A8 = 25000 microseconds or 40 pps
const bit<32> INTERVAL = 0xF42A4; // ~1 seconds (1000000 microseconds)
const bit<32> PACKETSININTERVAL = 1;
const bit<32> RESPONSETIME = 0x2DC6C0; // 3 seconds in microseconds
const bit<32> ALPHA = 1648;   
const bit<32> PACKETS_THRESHOLD = 100;  
...
control MyIngress(inout headers hdr,
    ...
    counter(MAX_TUNNEL_ID, CounterType.packets) MBReqCounter;
    ...
    action compute_funcClass_Id() {
        hash (
            ...
            (bit<1>)0,
            {
                hdr.ipv4.srcAddr,
                hdr.ipv4.dstAddr,
                //hdr.tcp.srcPort, // srcPort can be removed if Modbus Master
                hdr.tcp.dstPort, // changes the src port for each request
                hdr.modbus.functionCode
            },
            (bit<32>)65535
            );
        MBReqCounter.count((bit<32>) fClass_id);
    }
