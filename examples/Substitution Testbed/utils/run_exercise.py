...
class ExerciseRunner:
    ...
    def __init__(self, topo_file, log_dir, pcap_dir,
        ...
        self.substitutable = self.find_substitutable(self.links)
        print('The substitutable switches are', self.substitutable)
        self.linked_switches = self.create_virtual_interfaces(self.substitutable, self.links)
    ...
    def find_substitutable(self, links):
        host_neighbors = []
        potential_switches = []
        for link_ports in links:
            link = {'node1': link_ports['node1'].split('-')[0],
                    'node2': link_ports['node2'].split('-')[0]}
            print(link)
            if link['node1'][0] == 'h' or link['node1'][0] == 'h':
                if link['node1'][0] == 's':
                    if link['node1'] not in host_neighbors:
                        host_neighbors.append(link['node1'])
                if link['node2'][0] == 's':
                    if link['node2'] not in host_neighbors:
                        host_neighbors.append(link['node2'])
            else:
        ...
        substitutable_switches = []
        for switch in potential_switches:
            if switch not in host_neighbors:
                substitutable_switches.append(switch)
        return substitutable_switches
    
    def create_virtual_interfaces(self, substitutable, links):
        # Find all switches linked to substitutable switches
        linked_switches = []
        for link_ports in links:
            link = {'node1': link_ports['node1'].split('-')[0],
                    'node2': link_ports['node2'].split('-')[0]}
            if link['node1'] in substitutable:
                if link['node2'] not in linked_switches:
                    linked_switches.append(link['node2'])
            if link['node2'] in substitutable:
                if link['node1'] not in linked_switches:
